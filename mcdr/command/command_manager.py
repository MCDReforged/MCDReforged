...
class CommandManager:
	...
	def send_message(self, info, msg):
		self.mcdr_server.server_interface.reply(info, msg, is_plugin_call=False)
	...
	def process_mcdr_command(self, info):
		...
		# !!MCDR
		if len(args) == 1:
			self.send_help_message(info, self.tr('command_manager.help_message'))

		# !!MCDR reload
		elif len(args) >= 2 and args[1] in ['r', 'reload']:
			if len(args) == 2:
				self.send_help_message(info, self.tr('command_manager.help_message_reload'))
			elif len(args) == 3:
		...
		# !!MCDR permission
			...
			if len(args) == 2:
				self.send_help_message(info, self.tr('command_manager.help_message_permission'))
		...
		# !!MCDR plugin
			...
			if len(args) == 2:
				self.send_help_message(info, self.tr('command_manager.help_message_plugin'))
		...
		# !!MCDR checkupdate
			...
			self.mcdr_server.update_helper.check_update(reply_func=reply)
	...
	def function_call(self, info, func, name, func_args=(), success_message=True, fail_message=True, message_args=()):
		try:
			...
			if success_message:
				self.send_message(info, self.tr('command_manager.{}.success'.format(name), *message_args))
			return ret
		except:
			if fail_message:
				self.send_message(info, self.tr('command_manager.{}.fail'.format(name), *message_args))
	...
	def refresh_changed_plugins(self, info):
		ret = self.function_call(info, self.mcdr_server.plugin_manager.refresh_changed_plugins, 'refresh_changed_plugins', success_message=False)
		if ret is not None:
			self.send_message(info, ret.return_value)

	def reload_config(self, info):
		self.function_call(info, self.mcdr_server.load_config, 'reload_config')

	def reload_permission(self, info):
		self.function_call(info, self.mcdr_server.permission_manager.load_permission_file, 'reload_permission')
	...
	def set_player_permission(self, info, player, level):
		...
		if level is None:
			self.send_message(info, self.tr('command_manager.invalid_permission_level'))
		elif not Validator.player_name(player):
			self.send_message(info, self.tr('command_manager.invalid_player_name'))
		else:
			executor_level = self.mcdr_server.permission_manager.get_info_permission_level(info)
			if max(self.mcdr_server.permission_manager.format_level_value(level), self.mcdr_server.permission_manager.get_player_permission_level(player)) > executor_level:
				self.send_message(info, self.tr('command_manager.permission_not_enough'))
			else:
				self.mcdr_server.permission_manager.set_permission_level(player, level)
				if info.is_player:
					self.send_message(info, self.tr('permission_manager.set_permission_level.done', player, level))

	def remove_player_permission(self, info, player):
		if not Validator.player_name(player):
			self.send_message(info, self.tr('command_manager.invalid_player_name'))
		else:
			if self.mcdr_server.permission_manager.get_player_permission_level(player) > self.mcdr_server.permission_manager.get_info_permission_level(info):
				self.send_message(info, self.tr('command_manager.permission_not_enough'))
			else:
				self.mcdr_server.permission_manager.remove_player(player)
				self.send_message(info, self.tr('command_manager.remove_player_permission.player_removed', player))
	...
	def set_default_permission(self, info, level):
		...
		if level is None:
			self.send_message(info, self.tr('command_manager.invalid_permission_level'))
		elif self.mcdr_server.permission_manager.format_level_value(level) > self.mcdr_server.permission_manager.get_info_permission_level(info):
			self.send_message(info, self.tr('command_manager.permission_not_enough'))
		else:
			self.mcdr_server.permission_manager.set_default_permission_level(level)
			if info.is_player:
				self.send_message(info, self.tr('permission_manager.set_default_permission_level.done', level))
	...
	def print_mcdr_status(self, info):
		...
		if self.mcdr_server.permission_manager.get_info_permission_level(info) >= PermissionLevel.OWNER:
			self.send_message(info, RTextList(
	...
	def list_plugin(self, info):
		...
		self.send_message(info, self.tr('command_manager.list_plugin.info_loaded_plugin', len(file_list_loaded)))
		...
		self.send_message(info, self.tr('command_manager.list_plugin.info_disabled_plugin', len(file_list_disabled)))
		...
		self.send_message(info, self.tr('command_manager.list_plugin.info_not_loaded_plugin', len(file_list_not_loaded)))
	...
	def disable_plugin(self, info, file_name):
		...
		if not os.path.isfile(os.path.join(self.mcdr_server.plugin_manager.plugin_folder, file_name)):
			self.send_message(info, self.tr('command_manager.invalid_plugin_name', file_name))
		else:
			self.function_call(info, self.mcdr_server.plugin_manager.disable_plugin, 'disable_plugin', func_args=(file_name, ), message_args=(file_name, ))
	...
	def load_plugin(self, info, file_name):
		...
		if not os.path.isfile(os.path.join(self.mcdr_server.plugin_manager.plugin_folder, file_name)):
			...
			ret = self.function_call(
				info, self.mcdr_server.plugin_manager.load_plugin, 'load_plugin',
				func_args=(file_name, ), message_args=(file_name, ), success_message=False
			)
			if ret is not None:  # no outside exception
				self.send_message(info, self.tr('command_manager.load_plugin.{}'.format('success' if ret.return_value else 'fail'), file_name))
	...
	def enable_plugin(self, info, file_name):
		...
		if not os.path.isfile(os.path.join(self.mcdr_server.plugin_manager.plugin_folder, file_name)):
			...
			ret = self.function_call(
				info, self.mcdr_server.plugin_manager.enable_plugin, 'enable_plugin',
				func_args=(file_name, ), message_args=(file_name, ), success_message=False
			)
			if ret is not None:  # no outside exception
				if ret.return_value:
	...
	def reload_all_plugin(self, info):
		ret = self.function_call(info, self.mcdr_server.plugin_manager.refresh_all_plugins, 'reload_all_plugin', success_message=False)
		if ret is not None:
			self.send_message(info, ret.return_value)
	...
	def process_help_command(self, info):
		help_messages = [HelpMessage('!!MCDR', self.tr('command_manager.mcdr_help_message'), RText('MCDR', color=RColor.gold))]
		for plugin in self.mcdr_server.plugin_manager.get_plugins():
